...
# Build ICE server list — STUN always included; TURN loaded from Streamlit secrets if available.
# To add TURN support: in Streamlit Cloud → App settings → Secrets, add:
#   [turn]
...
try:
    _turn = st.secrets.get("turn", {})
    if _turn:
        _ice_servers.append({
            "urls":       list(_turn["urls"]),
            "username":   str(_turn["username"]),
            "credential": str(_turn["credential"]),
        })
except Exception:
    pass  # secrets not configured — STUN-only mode
